...
CREATE OR REPLACE TABLE `web3-publicgoods.ens.decoded_registrar_controller_NameRegistered_v3` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
  ...
  data,
  'v3' AS version
FROM `web3-publicgoods.ens.raw_registrar_controller_events`
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens.decoded_registrar_controller_NameRegistered_v4` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens.decoded_registrar_controller_NameRenewed` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
