...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp2.decoded_registrar_NameRegistered` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
...
FROM `web3-publicgoods.ens_temp2.raw_registrar_events`
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp2.decoded_registrar_NameRenewed` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp2.decoded_registrar_Transfer` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
