...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp.ens_decoded_registrar_event_NameRegistered` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
...
FROM `web3-publicgoods.ens_temp.ens_raw_registrar_events`
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp.ens_decoded_registrar_event_NameRenewed` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
...
CREATE OR REPLACE TABLE `web3-publicgoods.ens_temp.ens_decoded_registrar_event_Transfer` AS
SELECT
  transaction_hash,
  block_number,
  block_timestamp,
