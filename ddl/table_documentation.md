...
| node | BYTES | The ENS node hash associated with the resolver |
| addr | STRING | The resolved Ethereum address |
| texts | STRING | Array of known text record keys as a single string |
| addresses | STRING | Mapping of chain IDs and cointypes to addresses, stored as a string |
| contenthash | BYTES | The content hash for the node |
