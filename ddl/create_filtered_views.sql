...
CREATE OR REPLACE VIEW `web3-publicgoods.ens_temp2.view_registry` AS
WITH active_labelhashes AS (
  SELECT DISTINCT
    labelhash
  FROM `web3-publicgoods.ens_temp2.registration_periods`
  GROUP BY labelhash
...
CREATE OR REPLACE VIEW `web3-publicgoods.ens_temp2.view_resolvers` AS
...
SELECT 
  res.*
FROM `web3-publicgoods.ens_temp2.resolvers` res
WHERE 
...
CREATE OR REPLACE VIEW `web3-publicgoods.ens_temp2.view_resolutions` AS  
WITH active_labelhashes AS (
  SELECT DISTINCT
    labelhash
  FROM `web3-publicgoods.ens_temp2.registration_periods`
...
SELECT 
  res.*
FROM `web3-publicgoods.ens_temp2.resolutions` res
WHERE 
