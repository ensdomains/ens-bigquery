...
CREATE TEMP FUNCTION
  DECODE_RENEW_CONTROLLER_5_CALL(data STRING)
  RETURNS STRING
  LANGUAGE js AS """
  const abi = [
    "function renew(string label, uint256 duration, bytes32 referrer)"
  ];
  const interface = new ethers.utils.Interface(abi);
  const tx = interface.parseTransaction({data: data})
  if(tx === null) return null;
...
CREATE OR REPLACE TABLE `ens-manager.names.ETHRegistrarController4_event_NameRenewed_fixed` AS
  WITH to_controller_traces AS (
      SELECT * FROM `ens-manager.names.controller_traces`
      WHERE to_address = "0x253553366da8546fc250f225fe3d25d0c782303b"
  ),
  from_controller_traces AS (
      SELECT * FROM `ens-manager.names.controller_traces`
      WHERE from_address = "0x253553366da8546fc250f225fe3d25d0c782303b"
  ),
  renew_traces AS (
    SELECT 
      *,
  ...
  FROM `ens-manager.names.ETHRegistrarController4_event_NameRenewed` e
...
CREATE OR REPLACE TABLE `ens-manager.names.ETHRegistrarController5_event_NameRenewed_fixed` AS
  ...
  renew_traces AS (
    SELECT 
      *,
      DECODE_RENEW_CONTROLLER_5_CALL(input) AS label
    FROM to_controller_traces
    WHERE
